...
export interface SqlEditorRef {
    ...
    updateConnectId: (conn_id: string) => void;
}
...
const SqlEditor = forwardRef<SqlEditorRef, SqlEditorProps>((sqlEditorProps, ref) => {
        ...
        useImperativeHandle(ref, () => ({
            openFile, setEditorText, updateConnectId
        }));
        ...
        const updateConnectId = (value: string) => {
            setConnId(value)
        }

        const onConnectionIdChange = (value: string) => {
            setConnId(value)
            //notify parent when connection changed
            if (sqlEditorProps.onEditorConnectionChange) {
                sqlEditorProps.onEditorConnectionChange(value)
            }
        };
        ...
        const getToolbar = () => {
            ...
            return <Flex gap="small" justify={"left"} align={"flex-end"}>
                <Select size={"small"}
                        value={connId}
